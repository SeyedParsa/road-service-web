...
class ReportGenerator:
    ...
    @staticmethod
    def get_interval_report(region, start_time, end_time):
        ...
        while time < end_time:
            ...
            report.done_issue_counts.append(issues.filter(state=Issue.State.DONE).count())
    ...
    @staticmethod
    def get_distribution_report(region):
    ...
    @staticmethod
    def get_subregions_report(region):
        report = SubregionsReport(region)
        for subregioin in region.sub_regions.all():
            missions = subregioin.get_missions()
            issues = subregioin.get_issues()
            ...
            report.subregions_info.append((subregioin, mission_count, issue_count,
                                           mission_score_avg, mission_success_rate))
        return report
